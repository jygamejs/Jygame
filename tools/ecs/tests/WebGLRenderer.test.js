...
describe("WebGLRenderer — rasterized text", () => {
  it("renders a text entity as a single instanced quad", async () => {
    ...
    try {
      ...
      const renderer = new WebGLRenderer({ context: gl, width: 800, height: 600 });
      renderer.render(world);
      const quads = calls.drawArraysInstanced.filter((c) => c.count === 4);
      assert.strictEqual(quads.length, 1, "one instanced quad for the whole text");
      assert.strictEqual(quads[0].instanceCount, 1);
      renderer.destroy();
    } finally {
